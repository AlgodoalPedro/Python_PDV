def compRemProduto(cpf):  #exibe a tabela e pede para o usuário um código
    np = int(input("""
    +--------+------------------+---------+
    ...
    preco = "%s"%(tab_precos[np - 1]) #preço que será usado no calculo do valor
    
    valor = preco * qntd #valor total do produto
    
    prod = tab_produtos[np - 1]
    f = float(valor) #transformando o valor em float

    with open("%s.txt"%cpf, "a") as extrato:
        extrato.write("%s  -  %s Preço unitário:  %s  Valor:  - %.2f - Cancelado\n"%(qntd, prod, tab_printpreco[np - 1], f)) #escrevendo os pedidos no extrato
